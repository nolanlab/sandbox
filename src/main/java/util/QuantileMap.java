...
public class QuantileMap {
    ...
    private final int maxDP_Len = 10000;

    public static QuantileMap getQuantileMap(Dataset ds) {

        int len = Math.min(ds.getDatapoints().length, 10000);
        double step = ds.getDatapoints().length / len;

        Datapoint[] dp = new Datapoint[len];
        double di = 0;
        for (int i = 0; i < len; i++) {
            int idx = (int) di;
            dp[i] = ds.getDatapoints()[idx];
            di += step;
        }
        ...
        for (int i = 0; i < params.length; i++) {
            double paramVal[] = new double[dp.length];
            for (int j = 0; j < paramVal.length; j++) {
                paramVal[j] = dp[j].getVector()[i];
            }
            source[i] = paramVal;
        }
        double[][] target = new double[params.length][dp.length];
        for (int i = 0; i < params.length; i++) {
            double paramVal[] = new double[dp.length];
            double max = 0;
            for (int j = 0; j < paramVal.length; j++) {
