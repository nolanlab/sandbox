...
public class CovarianceMatrix {
    ...
    private static double calculateCovariance(double[] parameter1, double[] parameter2) {
        ...
        double covariance = 0;
        double mean1 = calculateMean(parameter1);
        double mean2 = calculateMean(parameter2);
        for (int i = 0; i < parameter1.length; i++) {
            double componenti = (parameter1[i] - mean1) * (parameter2[i] - mean2);
            covariance += componenti;
        }
        covariance /= parameter1.length;
        return covariance;
    }
